 ...
 * parser.ts
 *
 * Enhanced command-line argument parsing with validation and type coercion.
 * Provides structured parsing for the AIrchitect CLI commands.
 */
...
/**
 * Command context for parsed results
 */
...
/**
 * CLIParser class for parsing CLI arguments
 */
export class CLIParser {
  private argumentDefinitions: ArgumentDefinition[];
  private optionDefinitions: OptionDefinition[];
  private config: ParserConfig;

  /**
   * Creates a new CLIParser instance
   ...
   */
  public defineArgument(definition: ArgumentDefinition): void {
   ...
   */
  public defineArguments(definitions: ArgumentDefinition[]): void {
   ...
   */
  public defineOption(definition: OptionDefinition): void {
   ...
   */
  public defineOptions(definitions: OptionDefinition[]): void {
  ...
  private parseOption(
    ...
    if (arg.includes('=')) {
      ...
      if (!optionPart || valuePart === undefined) {
        throw new CLIError(
          `Invalid option format: ${arg}`,
          ErrorCategory.VALIDATION,
          ErrorSeverity.LOW,
    ...
    if (nextArg && !this.isOption(nextArg)) {
      const value = this.coerceType(nextArg, optionDef.type);
   ...
   */
  private findOptionDefinition(name: string): OptionDefinition | undefined {
   ...
   */
  private applyDefaults(result: ParsedArguments): void {
   ...
   */
  private validateValue(value: unknown, definition: ArgumentDefinition | OptionDefinition): void {
    // Check choices
