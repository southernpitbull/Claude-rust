...
export class LlamaIndexIntegration {
  ...
  constructor(credentialSystem: CredentialSystem, storageDir?: string) {
    this.credentialSystem = credentialSystem;
    this.storageDir = storageDir || './.aichitect/llamaindex';
  }
   ...
   */
  public async addDocument(content: string, metadata: LlamaIndexMetadata): Promise<boolean> {
   ...
   * Query the index
   */
  public async query(query: string, topK: number = 5): Promise<LlamaIndexSearchResult[]> {
    if (!this.isInitialized()) {
      throw new Error('LlamaIndex integration not initialized');
    }
  ...
  public async findSimilar(content: string, topK: number = 5): Promise<LlamaIndexSearchResult[]> {
    ...
    return await this.query(content, topK);
  }
